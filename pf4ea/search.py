...
class ReachGoal:
    def __init__(self, problem, heuristic, use_variant=False):
        ...
        self.f_score = lambda state: state.get_path_cost() + self.heuristic(state.node)
        self.path = None
        self.open = None
        self.closed = None
        self.wait = None
    ...
    def search(self):
        ...
        if self.use_variant:
            ...
            solution = self.ReachGoal_variant()
            return solution, time.time() - start_time
        else:
            start_time = time.time()
            solution = self.ReachGoal()
            return solution, time.time() - start_time 


    def ReconstructPath(self, init, goal_state):
        path = [goal_state.node]
        current_state = goal_state
        wait = 0
        while current_state.node != init.node:
            next_state = current_state.parent
            path.append(next_state.node)
        ...
        while self.open:
            ...
            for child_state in expand(self.problem, current_state):
                if child_state not in self.closed:
                    ...
                    traversable = is_collision_free(self.problem.agent_paths, current_node, child_node, time, self.problem.cols)
    ...
    def ReachGoal_variant(self):
        predecessors = self.heuristic.get_predecessors()
        ...
        while self.open:
            ...
            current_node = current_state.node
            path_free = is_free_path(self.problem, current_node, time, predecessors)
    ...
    def __str__(self):
        if self.path is None or self.open is None or self.closed is None or self.wait is None:
            ...
            return  (f"## RISULTATO DELLA RICERCA\n"
                    ...
                    f"  * **Costo del percorso:** {self.path_cost}\n"
